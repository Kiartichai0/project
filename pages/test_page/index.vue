  ...
  <v-card class="my-5" width="80%">
    ...
    <v-form ref="form" @submit.prevent="submit" >
      <v-container fluid>
        <v-row class="col-8" >
          <v-col cols="12" >
            <v-text-field v-model="form.username" :rules="rules.name" label="Username" required ></v-text-field>
          </v-col>
          <v-col cols="12" >
            <v-text-field :append-icon="show1 ? 'mdi-eye' : 'mdi-eye-off'" :type="show1 ? 'text' : 'password'" counter @click:append="show1 = !show1" v-model="form.password1" :rules="rules.name" label="Password" required ></v-text-field>
          </v-col>

          <v-col cols="12" >
            <v-text-field :append-icon="show1 ? 'mdi-eye' : 'mdi-eye-off'" :type="show1 ? 'text' : 'password'" counter @click:append="show1 = !show1" v-model="form.password2" :rules="rules.password" label="Password again" required ></v-text-field>
          </v-col>
          
          <v-col cols="12" sm="6" >
            <v-text-field v-model="form.first" :rules="rules.name" label="First name" required ></v-text-field>
          </v-col>

          <v-col cols="12" sm="6" >
            <v-text-field v-model="form.last" :rules="rules.name" label="Last name" required ></v-text-field>
          </v-col>

          <v-col cols="12" >
            <v-avatar v-if="form.pic != '' " size="100"> <img :src="form.pic" alt="avatar"/> </v-avatar>
            <v-text-field v-model="form.pic" label="avatar" ></v-text-field>
          </v-col>
          ...
          <v-col cols="12" >
            <v-select v-model="form.role" :items="roles" :rules="rules.role" color="pink" label="Role" required ></v-select>
          </v-col>
